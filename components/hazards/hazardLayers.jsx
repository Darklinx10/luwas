...
export default function HazardLayers({ activeHazard, map, setLoading }) {
  ...
  // Fetch hazard GeoJSON from Firestore
  // Fetch all hazard files under a hazard type
const fetchHazardFromFirebase = async (hazardType) => {
  try {
  ...
  useEffect(() => {
    ...
    const geoJsonLayers = {
      'Liquefaction': () => createGeoJsonLayer({
        ...
        infoText: 'Areas with potential for ground failure due to soil liquefaction during strong earthquakes.'
      }),
      'Earthquake Induced Landslide': () => createGeoJsonLayer({
        ...
        styleFn: (f) => styleBySusceptibility(normalizeSusceptibility(f.properties?.Susciptibi)),
        infoText: 'Areas susceptible to landslides triggered by earthquake ground shaking.'
      }),
      'Rain Induced Landslide': () => createGeoJsonLayer({
        ...
        styleFn: (f) => styleBySusceptibility(normalizeSusceptibility(f.properties?.Susciptibi)),
        infoText: 'Areas susceptible to landslides triggered by heavy or continuous rainfall.'
      }),
      'Tsunami': () => createGeoJsonLayer({
        hazardType: 'Tsunami',
        popupLabel: 'Tsunami Hazard Susceptibility',
        styleFn: tsunamiStyle,
        infoText: 'Tsunami-prone zones based on historical and model data.'
      }),
