...
import { fetchHazardFromFirebase } from "../app/(home)/(admin)/hazards/components/utils/fetchHazards";
import { normalizeSusceptibility } from "./susceptibility";
  ...
  useEffect(() => {
    ...
    const geoJsonLayers = {
      ...
      'Ground Shaking': () => createGeoJsonLayer({
        ...
        popupLabel: 'Ground Shaking Susceptibility',
        styleFn: (f) => styleBySusceptibility(normalizeSusceptibility(f.properties?.Susciptibi)),
        infoText: `
                    <p class="mb-2">Areas susceptible to strong ground movement during earthquakes.</p>
                    <p class="font-medium mb-1">Legend:</p>
                    <ul class="space-y-1">
                      <li class="flex items-center gap-2">
                        <span class="w-4 h-4 bg-red-600 inline-block border"></span> Highly Susceptible
                      </li>
                      <li class="flex items-center gap-2">
                        <span class="w-4 h-4 bg-purple-600 inline-block border"></span> Moderately Susceptible
                      </li>
                      <li class="flex items-center gap-2">
                        <span class="w-4 h-4 bg-yellow-400 inline-block border"></span> Least Susceptible
                      </li>
                      <li class="flex items-center gap-2">
                        <span class="w-4 h-4 bg-orange-400 inline-block border"></span> Generally Susceptible
                      </li>
                    </ul>
                  `,
        map, setLoading, updateInfo, geoJsonLayerRef
