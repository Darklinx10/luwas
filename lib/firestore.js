...
export const deleteData = async (collectionName, id) => {
  ...
  const subcollections = await (await getDocs(collection(docRef, '__dummy__'))).query.parent.listCollections();

  for (const subcol of subcollections) {
    const subDocs = await getDocs(subcol);
    for (const subDoc of subDocs.docs) {
      // Recursively delete each subdocument
      await deleteData(`${collectionName}/${id}/${subcol.id}`, subDoc.id);
    }
  }

  // Delete the parent document after subcollections are deleted
