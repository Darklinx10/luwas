...
export function analyzeFirestoreError(error, queryInfo = {}) {
  ...
  // Firebase provides a pre-filled composite index creation URL in the error
  // Look for URLs in format: https://console.firebase.google.com/v1/r/project/{projectId}/firestore/indexes?create_composite={encodedIndexConfig}
  let indexUrl = null;
  
  // Use a more aggressive pattern that captures the full URL including query parameters
  ...
  for (const {source, label} of errorSources) {
    ...
    if (matches && matches.length > 0) {
      ...
      if (indexUrl) {
        // Log which source provided the URL for debugging
  ...
  if (!indexUrl) {
    ...
    if (projectId) {
      // Use the actual project ID to generate the console link
      // Format: https://console.firebase.google.com/u/7/project/{projectId}/firestore/databases/-default-/indexes
      indexUrl = `https://console.firebase.google.com/u/0/project/${projectId}/firestore/databases/-default-/indexes`;
    } else {
