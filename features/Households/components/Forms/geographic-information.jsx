...
export default function GeographicIdentification({ householdId, goToNext }) {
      ...
      try {
        ...
        if (geoSnap.exists()) {
          ...
          setForm(prev => ({
            ...prev,
            ...data,
            // Ensure homes has proper structure
            homes: data.homes && data.homes.length > 0 
              ? data.homes 
              : [{ label: 'Primary Home', latitude: '', longitude: '' }],
          }));
        }
      } catch (error) {
        console.error('Error loading existing geographic data:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save to geographicIdentification subcollection
      const geoRef = doc(db, 'households', householdId, 'geographicIdentification', 'main');
      await setDoc(geoRef, { ...form, uid: user.uid }, { merge: true });

      // Also save top-level household fields to align with upload/import schema
      // Extract and filter homes to only those with both latitude and longitude
