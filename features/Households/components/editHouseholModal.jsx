...
export default function EditHouseholdModal({ open, onClose, householdId, onUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Filter homes to only those with both latitude and longitude
      const homesWithCoords = geoFields.homes.filter(h => h.latitude && h.longitude);
      ...
      // Also update top-level household doc to maintain schema alignment
      ...
      const topLevelData = {
        ...
        headSex,
        headAge: headAge ? Number(headAge) : 0,
        contactNumber,
        barangay: geoFields.barangay || '',
        sitio: geoFields.sitio || '',
        homes: homesWithCoords,
        updatedAt: new Date(),
      };
      await updateDoc(hhRef, topLevelData);
      ...
      // If no members exist, also save head info in geo doc
      if (!headFound) {
        await updateDoc(geoRef, {
          headFirstName: capitalizeWords(headFirstName),
          headMiddleName: capitalizeWords(headMiddleName),
