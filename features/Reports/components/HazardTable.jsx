...
import { generateHazardCSV, downloadCSV } from '../utils/csvExport';
...
export default function HazardTable({
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((h) => {
      const haystack = `${h.name} ${h.barangay} ${h.contactNumber} ${h[key]}`
        .toLowerCase();
  ...
  const handleDownloadCSV = () => {
    ...
    const csv = generateHazardCSV(filteredData, title);
    downloadCSV(csv, `hazard_reports_${new Date().getFullYear()}.csv`);
  };
  ...
  return (
    <div className="p-4">
                ...
                <tbody>
                  {filteredData.map((h, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 border">{h.homeLabel}</td>
                      <td className="px-4 py-2 border">
                        {formatValue(
                          h[legendProp?.key || 'Unknown']
                        )}
                      </td>
                    </tr>
                  ))}
      ...
      <p className="text-sm text-gray-700 mt-4">
        <strong>Total Households Affected:</strong> {filteredData.length}
      </p>
    </div>
  );
}
